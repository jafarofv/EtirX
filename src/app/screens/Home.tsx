...
export function Home() {
  ...
  const isWomen = (id: number) => [1, 3, 5].includes(id);
  const isMen = (id: number) => [2, 4].includes(id);
  const isUnisex = (id: number) => [6].includes(id);
  const isNew = (id: number) => [4, 5, 6].includes(id);
  const filteredPerfumes = perfumes.filter(
    (p) =>
      (selectedCategory === "all" ||
        (selectedCategory === "new" && isNew(p.id)) ||
        (selectedCategory === "women" && isWomen(p.id)) ||
        (selectedCategory === "men" && isMen(p.id)) ||
        (selectedCategory === "unisex" && isUnisex(p.id)) ||
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="px-4 sm:px-6 lg:px-8 mb-8">
        <div className="bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-900 rounded-3xl p-6 border border-zinc-700 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <button className="bg-white text-black px-6 py-2.5 rounded-full text-sm font-medium hover:bg-zinc-100 transition-all">
              {t("home.offer.cta")}
            </button>
          </div>
        </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => navigate("/shop")} className="text-sm text-zinc-400 hover:text-white transition-colors">
            {t("home.viewAll")}
          </button>
        </div>
